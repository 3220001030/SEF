...
# --- Configuration ---
base_dir = "/Users/gurumakaza/Documents/data/IPIO_DHF"
years_known = [2002, 2007, 2012, 2017]
years_all = list(range(2002, 2023))  # if you need for future steps
globals_dict = globals()
...
for file in os.listdir(base_dir):
    ...
    if any(f"_{year}_" in file for year in years_known):
        try:
            ...
            # Select the sub-DataFrame (excluding first 5 rows and columns)
            target = df.iloc[5:, 5:]

            # Force conversion to numeric (non-numeric becomes NaN), then multiply
            df.iloc[5:, 5:] = target.applymap(
                lambda x: np.log1p(pd.to_numeric(x, errors='coerce') * 1e4)
                )
...
output_dir = "/Users/gurumakaza/Documents/data/IO table/cleaned_csv/BTS"
# Ensure output directory exists

os.makedirs(output_dir, exist_ok=True)
# Generate values: 1 to 42 repeated 9 times = 378 values
sequence = list(range(1, 43)) * 9  # 1–42 × 9 = 378



# Loop through years and save each DataFrame
for year in range(2002, 2023):
    df_name = f'iot_{year}_df'
    df = globals().get(df_name)
    
    # Fill row 3, from column 5 onward
    df.iloc[2, 5:5+len(sequence)] = sequence

    # Fill column 3, from row 5 downward
    df.iloc[5:5+len(sequence), 2] = sequence

    
    
    if df is not None:
        df.to_csv(os.path.join(output_dir, f'iot_{year}_df.csv'), 
...
# Set your data folder
data_folder = '/Users/gurumakaza/Documents/data/IO table/cleaned_csv/BTS'
...
for file_path in tqdm(csv_files, desc="Processing files", unit="file"):
    ...
    # STEP 2: Extract all unique city names (from earlier)
    row_cities = df_cleaned.iloc[5:, 0].dropna().astype(str).unique()
    col_cities = df_cleaned.iloc[0, 5:].dropna().astype(str).unique()
    all_cities = pd.unique(pd.Series(list(row_cities) + list(col_cities)))
    all_cities = [c.strip() for c in all_cities if c.strip() != ""]
    all_cities.sort()
    # Clean up
...
# Step 5: Save cleaned version
cleaned_path = "/Users/gurumakaza/Documents/data/Correlation BTS.csv"
df.to_csv(cleaned_path, index=False)
print(f"✅ Cleaned and saved to {cleaned_path}")
...
ppmlhdfe realGDP lnK lnL if coastal == 1, absorb (年份)
...
ppmlhdfe realGDP lnK lnL 内资ESP_QoL c.内资ESP_QoL#c.lnK c.内资ESP_QoL#c.lnL if coastal == 1, absorb (年份 城市)
outreg2 using SEF_base_coastal.doc, append alpha(0.001, 0.01, 0.05) bdec(3) tdec(3) ctitle(1) addstat(Pseudo R-squared, `e(r2_p)') addtext(Province City Year FE, YES)

ppmlhdfe realGDP lnK lnL 内资ESP_QoL c.内资ESP_QoL#c.lnK c.内资ESP_QoL#c.lnL 内资WI_RDS c.内资WI_RDS#c.lnK c.内资WI_RDS#c.lnL if coastal == 1, absorb (年份 城市)
...
ppmlhdfe realGDP lnK lnL 内资ESP_QoL c.内资ESP_QoL#c.lnK c.内资ESP_QoL#c.lnL 内资WI_RDS c.内资WI_RDS#c.lnK c.内资WI_RDS#c.lnL c.内资ESP_ES c.内资ESP_ES#c.lnK c.内资ESP_ES#c.lnL if coastal == 1, absorb (年份 城市)
...
outreg2 using SEF_base_coastal.doc, append alpha(0.001, 0.01, 0.05) bdec(3) tdec(3) ctitle(2002-2007) addstat(Pseudo R-squared, `e(r2_p)') addtext(Province City Year FE, YES)
...
outreg2 using SEF_base_coastal.doc, append alpha(0.001, 0.01, 0.05) bdec(3) tdec(3) ctitle(2008-2020) addstat(Pseudo R-squared, `e(r2_p)') addtext(Province City Year FE, YES)
...
ppmlhdfe realGDP lnK lnL 自然增长率 内资ESP_QoL c.内资ESP_QoL#c.lnK c.内资ESP_QoL#c.lnL 内资WI_RDS c.内资WI_RDS#c.lnK c.内资WI_RDS#c.lnL 内资ESP_ES c.内资ESP_ES#c.lnK c.内资ESP_ES#c.lnL 内资WI_ES c.内资WI_ES#c.lnK c.内资WI_ES#c.lnL 内资RDS_QoL if coastal == 1, absorb (年份 城市)
outreg2 using SEF_base_coastal.doc, append alpha(0.001, 0.01, 0.05) bdec(3) tdec(3) ctitle(4) addstat(Pseudo R-squared, `e(r2_p)') addtext(Province City Year FE, YES)

ppmlhdfe realGDP lnK lnL 自然增长率 内资ESP_QoL c.内资ESP_QoL#c.lnK c.内资ESP_QoL#c.lnL 内资WI_RDS c.内资WI_RDS#c.lnK c.内资WI_RDS#c.lnL 内资ESP_ES c.内资ESP_ES#c.lnK c.内资ESP_ES#c.lnL 内资WI_ES c.内资WI_ES#c.lnK c.内资WI_ES#c.lnL 内资RDS_QoL c.内资RDS_QoL#c.lnK c.内资RDS_QoL#c.lnL if coastal == 1, absorb (年份 城市)
outreg2 using SEF_base_coastal.doc, append alpha(0.001, 0.01, 0.05) bdec(3) tdec(3) ctitle(4) addstat(Pseudo R-squared, `e(r2_p)') addtext(Province City Year FE, YES)

ppmlhdfe realGDP lnK lnL 自然增长率 内资ESP_QoL c.内资ESP_QoL#c.lnK c.内资ESP_QoL#c.lnL 内资WI_RDS c.内资WI_RDS#c.lnK c.内资WI_RDS#c.lnL 内资ESP_ES c.内资ESP_ES#c.lnK c.内资ESP_ES#c.lnL 内资WI_ES c.内资WI_ES#c.lnK c.内资WI_ES#c.lnL 内资RDS_QoL c.内资RDS_QoL#c.lnK c.内资RDS_QoL#c.lnL c.内资RDS_QoL#c.内资ESP_QoL if coastal == 1, absorb (年份 城市)
outreg2 using SEF_base_coastal.doc, append alpha(0.001, 0.01, 0.05) bdec(3) tdec(3) ctitle(4) addstat(Pseudo R-squared, `e(r2_p)') addtext(Province City Year FE, YES)
```
```Python
erase SEF_base_coastal.txt  
...
ppmlhdfe realGDP lnK lnL 自然增长率 内资ESP_QoL c.内资ESP_QoL#c.lnK c.内资ESP_QoL#c.lnL 内资WI_RDS c.内资WI_RDS#c.lnK c.内资WI_RDS#c.lnL 内资ESP_ES c.内资ESP_ES#c.lnK c.内资ESP_ES#c.lnL 内资WI_ES c.内资WI_ES#c.lnK c.内资WI_ES#c.lnL 内资RDS_QoL c.内资RDS_QoL#c.lnK c.内资RDS_QoL#c.lnL 内资QoL_LoD if coastal == 1, absorb (年份 城市)
outreg2 using SEF_base_coastal.doc, append alpha(0.001, 0.01, 0.05) bdec(3) tdec(3) ctitle(4) addstat(Pseudo R-squared, `e(r2_p)') addtext(Province City Year FE, YES)

ppmlhdfe realGDP lnK lnL 自然增长率 内资ESP_QoL c.内资ESP_QoL#c.lnK c.内资ESP_QoL#c.lnL 内资WI_RDS c.内资WI_RDS#c.lnK c.内资WI_RDS#c.lnL 内资ESP_ES c.内资ESP_ES#c.lnK c.内资ESP_ES#c.lnL 内资WI_ES c.内资WI_ES#c.lnK c.内资WI_ES#c.lnL 内资RDS_QoL c.内资RDS_QoL#c.lnK c.内资RDS_QoL#c.lnL 内资QoL_LoD c.内资RDS_QoL#c.内资QoL_LoD if coastal == 1, absorb (年份 城市)
outreg2 using SEF_base_coastal.doc, append alpha(0.001, 0.01, 0.05) bdec(3) tdec(3) ctitle(4) addstat(Pseudo R-squared, `e(r2_p)') addtext(Province City Year FE, YES)
```
3rd order
```Python
